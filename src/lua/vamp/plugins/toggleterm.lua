return {
	...
	config = function(_, opts)
		...
		for index = 1, 9, 1 do
			...
			vim.keymap.set("n", "<leader>r" .. keymap_index .. "%", function()
				set_op_func(function(motion_type)
					toggleterm.send_lines_to_terminal(motion_type, true, { args = terminal_index })
				end)
			...
			vim.keymap.set("n", "<leader>r" .. keymap_index .. "l", function()
				toggleterm.send_lines_to_terminal("single_line", true, { args = terminal_index })
			end, { desc = "Send line to terminal", noremap = true })

			vim.keymap.set("x", "<leader>r" .. keymap_index .. "l", function()
				toggleterm.send_lines_to_terminal("visual_lines", true, { args = terminal_index })
			...
			vim.keymap.set("x", "<leader>r" .. keymap_index .. "s", function()
				toggleterm.send_lines_to_terminal("visual_selection", true, { args = terminal_index })
		...
		vim.keymap.set({ "n", "x" }, "<leader>rr", "<cmd>TermSelect<cr>", { desc = "Select terminal", noremap = true })
